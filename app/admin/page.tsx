...
type StatusFilter = "NEW" | "CONTACTED" | "CLOSED" | "ARCHIVED" | undefined;
...
export default async function AdminDashboard(props: {
  ...
  const [totalLeads, newLeads, contactedLeads, closedLeads, archivedLeads] = await Promise.all([
    ...
    <main className="min-h-screen bg-[#020617] text-white px-6 py-10">
      <div className="max-w-7xl mx-auto space-y-10">
        <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <a href="/admin/export" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm font-mono hover:bg-emerald-500/20 transition">
            Exportar CSV
          </a>
        </header>
        ...
        <section className="flex flex-wrap gap-3">
          ...
          <FilterButton label="Arquivados" href="/admin?status=ARCHIVED" active={statusFilter === "ARCHIVED"} />
        </section>
        ...
        <section className="space-y-4">
          ...
          {leads.length === 0 ? (
            ...
            <div className="space-y-4">
              {leads.map((lead) => (
                <div key={lead.id} className="bg-slate-900/50 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    ...
                    <form action={updateLeadStatus} className="flex items-center gap-2 shrink-0">
                      ...
                      <select name="status" defaultValue={lead.status} className="bg-slate-900 border border-white/10 rounded px-2 py-1 text-sm">
                        <option value="NEW">Novo</option>
                        <option value="CONTACTED">Contato feito</option>
                        <option value="CLOSED">Fechado</option>
                        <option value="ARCHIVED">Arquivado (teste)</option>
                      </select>
                      <button type="submit" className="px-3 py-1 text-xs rounded bg-blue-500/10 border border-blue-500/30 text-blue-400 hover:bg-blue-500/20 transition">
                        Salvar
                      </button>
                    </form>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <a href={"mailto:" + lead.email} className="px-3 py-1 text-xs rounded bg-blue-500/10 border border-blue-500/30 text-blue-400 hover:bg-blue-500/20 transition">
                      Responder por e-mail
                    </a>
                    <a href={"https://wa.me/55" + lead.phone.replace(/\D/g, "")} target="_blank" rel="noopener noreferrer" className="px-3 py-1 text-xs rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 transition">
                      WhatsApp
                    </a>
                  </div>
                </div>
...
function FilterButton({ label, href, active }: { label: string; href: string; active: boolean }) {
  return (
    <a href={href} className={"px-4 py-2 rounded-lg text-sm font-mono border transition " + (active ? "bg-blue-500/20 border-blue-500 text-blue-400" : "bg-slate-900/40 border-white/10 text-slate-400 hover:bg-white/5")}>
      {label}
    </a>
  );
}
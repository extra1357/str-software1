...
// ── DADOS ─────────────────────────────────────────────────────────────────────
// SERVICOS e REGIOES agora vêm de lib/servicos-data.ts e lib/regioes-data.ts
// (fonte única — as páginas /servicos/[slug] e /regioes/[slug] usam os mesmos
// arquivos, então não há mais risco de duplicação ficar dessincronizada).
...
function ServicoCard({ icone, titulo, descricao, slug, tags }: typeof SERVICOS[0]) {
  return (
    <Link href={`/servicos/${slug}`} className="group relative border border-white/8 p-7 bg-[#0d0d0d] hover:border-[#C8922A]/40 hover:bg-[#111] transition-all duration-300 block">
              ...
              <div>
                ...
                <ul className="space-y-2.5">
                  {SERVICOS.map(({ titulo, slug }) => (
                    <li key={titulo}><Link href={`/servicos/${slug}`} className="text-white/32 text-sm hover:text-white transition-colors">{titulo}</Link></li>
                  ))}
                </ul>
              </div>
              <div>
